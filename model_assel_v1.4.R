...
# v1.4
# temperature scenario option added
# toxic exposure scenario option added
# temperature dependent guts parameters
# food availability over time can be shown when using Monte-Carlo


# set random number generation to a fixed number (for testing and debugging purposes only)
#set.seed(123)
...
temp.v <- read.csv("/Users/dino/Dropbox/Uni/Master/Masterarbeit/Daten/Temperatur/temp_mean.csv", header = TRUE, sep = ";", dec = ".")$mean_temp
...
#sex.v <- c(rep('f', 205), rep('m', 50), rep('f', 290), rep('m', 205), rep('f', 30), rep('m', 220)) # field scenario
sex.v <- c(rep('f', 8), rep('m', 5), rep('f', 32), rep('m', 32), rep('f', 3), rep('m', 20)) # 1/10 m^2 field scenario
...
tox.on <- F
# Use scenario (= T) or enter start and end day (= F)
tox.scn <- F
...
## Monte Carlo
# matrices for each monte carlo step showing population size and feeding
mc.pop.size <- matrix(data = NA, nrow = days + 1, ncol = mc.no)
mc.pop.feed <- matrix(data = NA, nrow = days + 1, ncol = mc.no)
mc.env.food <- matrix(data = NA, nrow = days + 1, ncol = mc.no)
...
if (mc.on == FALSE) {
  ...
  m.ind <-cbind(L,Px,pxm,pa,f,UE,e,sl,Sc,dL,UH,UR,R,breeding,sex,b.L,b.e,b.UH,b.Sc,q,h,D_e,Fe,s,rs,ci,Ft,ht,alive,age)
  m.food <- cbind(c(0:(days)), pop.feed, env.foodav[1:c(days + 1)])
  colnames(m.food) <- c("day", "consumed", "available")
  
  plot(m.pop.size[, 1], m.pop.size[, 3], type = "l", xlab = "time (d)", ylab = "population size (#)", ylim = c(0,max(m.pop.size[ ,3])), cex.axis = 1.2, cex.lab = 1.2)
...
write.csv(mc.pop.feed, file = paste0(format(Sys.time(), "%Y-%m-%d_%H.%M"),"_simresults_","MC=",mc.no,"_tox=0.135single_food=std_temp=std", "_popfeed", ".csv", sep = ""))
write.csv(mc.env.food, file = paste0(format(Sys.time(), "%Y-%m-%d_%H.%M"),"_simresults_","MC=",mc.no,"_tox=0.135single_food=std_temp=std", "_foodav", ".csv", sep = ""))
